...
$base_path = ($script_name === DIRECTORY_SEPARATOR || $script_name === '.') ? '/' : $script_name . '/';
...
if (strpos($path, 'bsol/') === 0) {
    ...
    if (!isset($_SESSION['user_id'])) {
        $_SESSION['messages']['errors'][] = "Accès réservé : vous devez être connecté.";
    ...
    if (strpos($path, 'bsol/Progresser/') !== false && !in_array($user_rang, ['progresser', 'peaufiner', 'confirmer'])) {
        $can_access = false;
        $required_level = "Progresser";
    } elseif (strpos($path, 'bsol/Peaufiner/') !== false && !in_array($user_rang, ['peaufiner', 'confirmer'])) {
        $can_access = false;
        $required_level = "Peaufiner";
    } elseif (strpos($path, 'bsol/Confirmer/') !== false && $user_rang !== 'confirmer') {
        // Cas particulier : Peaufiner a accès à certains sous-dossiers de Confirmer
        if ($user_rang === 'peaufiner' && (strpos($path, 'confirmerProgresser') !== false || strpos($path, 'confirmerPeaufiner') !== false)) {
            $can_access = true;
        } else {
            $can_access = false;
    ...
    $file_path = $path;
    if (substr($file_path, -4) !== '.php') $file_path .= '.php';
...
if (in_array($path, $private_pages) && !isset($_SESSION['user_id'])) {
    $_SESSION['messages']['errors'][] = "Vous devez être connecté pour accéder à cette page.";
    header('Location: ' . BASE_URL . 'login');
    exit();
}
