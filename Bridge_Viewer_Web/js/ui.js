 ...
 */
function updateUI() {
  if (!window.gameStates || window.gameStates.length === 0) return;
  ...
  if (state.bidding.length > 0) {
    ...
    let startIndex = PLAYERS.indexOf(state.dealer); // La grille commence avec le donneur
      ...
      suits.forEach((suit) => {
        ...
        if (cardsOfSuit.length > 0) {
          ...
          cardsOfSuit.forEach((c) => {
            rowDiv.appendChild(
              createBsolCard(c.rank, SUITS[suit].color, false)
            );
          });
  ...
  state.currentTrick.forEach((play) => {
    let cardEl = createTrickCardElement(play.card);
