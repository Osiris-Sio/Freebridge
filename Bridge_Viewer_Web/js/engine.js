...
function simulateGame(parsed) {
  ...
  if (state.contract) {
    let match = state.contract.match(/(\d)(N|S|H|D|C)/i);
    if (match)
      state.trump =
  ...
  for (let i = 0; i < parsed.tokens.length; i++) {
    ...
    let newState = JSON.parse(JSON.stringify(states[states.length - 1]));
    ...
    if (t.type === 'comment') {
      ...
      if (newState.phase === 'bidding') {
        ...
        let nonPasses = newState.bidding.filter(
          (b) => b.bid.toUpperCase() !== 'P' && b.bid.toUpperCase() !== 'PASS'
        );
        let last3 = newState.bidding.slice(-3);
        let last3Pass =
          last3.length === 3 &&
          last3.every(
            (b) => b.bid.toUpperCase() === 'P' || b.bid.toUpperCase() === 'PASS'
          );
        let last4 = newState.bidding.slice(-4);
        let last4Pass =
          last4.length === 4 &&
          last4.every(
            (b) => b.bid.toUpperCase() === 'P' || b.bid.toUpperCase() === 'PASS'
        ...
        if (auctionOver) {
          ...
          if (nonPasses.length > 0) {
            let lastBid = nonPasses[nonPasses.length - 1];
            newState.contract = lastBid.bid;
            let match = newState.contract.match(/(\d)(N|S|H|D|C)/i);
            if (match) {
              newState.trump =
                match[2].toUpperCase() === 'N' ? null : match[2].toUpperCase();
            }
            ...
            let denom = match ? match[2].toUpperCase() : null;
            let team = ['N', 'S'].includes(lastBid.player)
              ? ['N', 'S']
              : ['E', 'W'];
            let teamBids = nonPasses.filter(
              (b) =>
          ...
          if (newState.declarer) newState.turn = getLHO(newState.declarer);
        }
      }
      states.push(newState);
    } else if (t.type === 'play') {
      // Retirer la carte de la main du joueur
      let hand = newState.hands[newState.turn];
      let cardIdx = hand
        ? hand.findIndex(
      ...
      if (cardIdx === -1 && newState.currentTrick.length === 0) {
        let playTokenIndices = [];
        for (
          let j = i;
          j < parsed.tokens.length && playTokenIndices.length < 4;
          j++
        ) {
          if (parsed.tokens[j].type === 'play') playTokenIndices.push(j);
        }

        if (playTokenIndices.length === 4) {
          // Déterminer les vrais propriétaires des 4 cartes
          let cards = playTokenIndices.map((idx) => parsed.tokens[idx].value);
          let owners = [];
          for (let c of cards) {
          ...
          if (owners.every((o) => o) && new Set(owners).size === 4) {
            ...
            let newCardValues = [];
            for (let expectedPlayer of expectedOrder) {
              let idx = owners.indexOf(expectedPlayer);
      ...
      if (newState.currentTrick.length === 4) {
        let leadSuit = newState.currentTrick[0].card.suit;
        ...
        for (let k = 1; k < 4; k++) {
          let c = newState.currentTrick[k].card;
          let p = newState.currentTrick[k].player;
          let val = cardValue(c.rank);
        ...
        if (['N', 'S'].includes(winner)) newState.tricksWon.NS++;
        else newState.tricksWon.EW++;
        ...
        let clearedState = JSON.parse(JSON.stringify(newState));
