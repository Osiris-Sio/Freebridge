...
function cacheMakeable(pindex, data) {
  ...
  try {
    ...
    const deal = board.Deal
    const dealstr =
      'W:' + deal[3] + 'x' + deal[0] + 'x' + deal[1] + 'x' + deal[2]
...
function getIndexedDDTricks(msg) {
  if (g_db == null) {
    ...
    const transaction = g_db.transaction(['ddCache'], 'readwrite')
    const objectStore = transaction.objectStore('ddCache')
    const req = objectStore.get(
      makeDealKey(msg.dealstr, msg.vulstr, msg.leadstr),
    )
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 5; j++) {
      ...
      rows[1 + i].cells[1 + 4 - j].onclick = function () {
        ...
        const indx =
          5 * ($(this.parentNode).index() - 1) + ($(this).index() - 1)
...
function getMakeableTricksForContract(index, contract, declarer) {
  ...
  const value = trickChars.indexOf(
    cvector.charAt(5 * decl + suit).toUpperCase(),
  )
    ...
    for (j = 0; j < tlines.length; j++) {
      ...
      if (played(tlines[j])) // Otherwise this pair didn't play this board.
      {
        ...
        if (nplayed > 0) {
          ...
          if (
            typeof g_travellers.event.participants.pair[nsIndex]
              .boardsPlayed === 'undefined'
          ) {
          ...
          if (
            typeof g_travellers.event.participants.pair[ewIndex]
              .boardsPlayed === 'undefined'
          ) {
  ...
  lines.sort(function (a, b) {
    ...
    if ((a.contract != null) & (b.contract != null)) {
      ...
      if (levelb > levela) {
        ...
        declarers.indexOf(b.played_by) > declarers.indexOf(a.played_by)
      ) {
        return 1
      } else if (
        declarers.indexOf(a.played_by) > declarers.indexOf(b.played_by)
      ) {
        return -1
      }
    }
...
function displayTraveller(pdirection) {
  ...
  if (typeof optimum !== 'undefined') {
    ...
    for (k = 0; k < optcontracts.length; k++) {
      ...
      const ctricks = getMakeableTricksForContract(
        g_lastBindex,
        contract,
        optdir.charAt(0),
      )
                    ...
                    if (
                      ...
                      if (ETFMode != null) {
                        if (
                          ...
                          const ltricks = getMakeableTricksForLead(
                            hindex,
                            tline,
                          )
    ...
    for (i = 0; i < g_travellers.event.board.length; i++) {
      if (g_travellers.event.board[i].board_no == g_hands.boards[j].board) {
        ...
        for (k = 0; k < tlines.length; k++) {
          ...
          if (tline.tricks != '') {
            const ddmakeable = getMakeableTricksForContract(
              j,
              tline.contract,
              tline.played_by,
            )
...
function checkContract(bindex, trindex) {
  ...
  if (validContract(line.contract)) {
    ...
    if (line.tricks >= 7) // They made a contract in this suit, is it likely ?
    {
      ...
      const oppTks1 = getMakeableTricksForContract(
        bindex,
        line.contract,
        decl.charAt(oppIndx1),
      )
      const oppTks2 = getMakeableTricksForContract(
        bindex,
        line.contract,
