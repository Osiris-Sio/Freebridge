...
function convertXML(data) {
  ...
  if (
    ...
    json.event.event_type = 'Teams';
  else if (json.event.event_type == 'SWISS_TEAMS') json.event.event_type = 'Swiss_Teams';
  else json.event.event_type = 'Pairs';
  ...
  if ((json.event.event_type == 'Teams') | (json.event.event_type == 'Swiss_Teams')) {
    ...
    if (ximpnodes.length != 0) tximps = true; // This event contains cross imp information for a Teams event
  }
  ...
  if (json.event.winner_type.length == 0) json.event.winner_type = 1;
  else json.event.winner_type = Number(json.event.winner_type);
  ...
  for (i = 0; i < pnodes.length; i++) {
    ...
    if (json.event.winner_type == 1) cpair.direction = 'N';
    else {
  ...
  json.event.board.sort(function (a, b) {
    try {
      if (Number(a.board_no) > Number(b.board_no)) return 1;
      else if (Number(a.board_no) == Number(b.board_no)) return 0;
      else return -1;
    } catch (ex) {
      return 0;
