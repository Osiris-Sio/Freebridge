  ...
  if ($user_id && $new_rang) {
    if ($new_rang === $old_rang) {
      $_SESSION['messages']['confirm'][] =
        'Aucun changement détecté pour cet utilisateur.';
    } else {
      try {
        if (!$conn) {
        ...
        $_SESSION['messages']['confirm'][] =
          "Le rang de l'utilisateur a été mis à jour avec succès.";
      } catch (Exception $e) {
        $_SESSION['messages']['errors'][] =
...
$sql =
  'SELECT user_id, user_nom, user_prenom, user_mail, user_rang, user_date, is_admin FROM user WHERE 1=1';
$params = [];

if (!empty($search)) {
  $sql .=
    ' AND (user_nom LIKE :search OR user_prenom LIKE :search OR user_mail LIKE :search)';
