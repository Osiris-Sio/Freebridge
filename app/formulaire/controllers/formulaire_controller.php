...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($nom) && !empty($prenom) && !empty($mail) && !empty($message)) {
    ...
    $subject =
      'Demande ' . $nom . ' ' . $prenom . ' via le formulaire de freebridge';

    $body =
      'La demande faite est la suivante :<br><br>' .
      nl2br(htmlspecialchars($message));
    $body .=
      "<br><br>Avec l'adresse mail suivante : " . htmlspecialchars($mail);

    $headers =
      'From: ' . mb_encode_header_object("$nom $prenom", $mail) . "\r\n";
