...
if ($number_of_rows==0) {
	$sql = $conn->prepare("INSERT INTO user(user_nom,user_prenom,user_rang,user_mail,user_password, user_date) VALUES ('".$nom."','".$prenom."','debutant','".$mail."','".$password."','".$date."')");
	$sql->execute();
	?>
	...
	<?php
	echo '<script>setTimeout(function() { document.location.href="login.php"}, 3000)</script>';
} else {
	?>
	<main class="mdl-layout__content">
    		<div class="page-content">
	...
	<?php
	echo '<script>setTimeout(function() { document.location.href="register.php"}, 3000)</script>';
